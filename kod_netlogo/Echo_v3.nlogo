...
to setup
  ...
  set replicate-chance random-float 10     ;; w Echo było jako suwak
...
to setup-patches
  ask patches [
    ifelse pycor < 0 [
      ; More likely to have resource1 in the bottom half (south)
      ...
      ; More likely to have resource2 in the top half (north)
      ifelse random-float 1 < 0.85 [
        set max-resource2 random 50
        set resource2 max-resource2
        set assigned-resource "resource2"
...
to setup-creatures
  ...
  [
    ...
    set size 0.1   ;; since the patch size is so big make the turtles smaller
...
to recolor-turtle  ;; turtle procedure
  ...
  set label reduce [ [a b] -> word a b ] (sentence mating "." offense "." defense)
end

to-report tag-color [tag]
  ;; report 5 + 2 * position tag all-tags ;; oryginalna wersja z Echo
  report 5 + 10 * ( count-letter "a" mating + 1) / ( count-letter "b" mating + 1)
end
