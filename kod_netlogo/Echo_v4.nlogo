...
patches-own [
  resource1         ;; the amount of resource1 a patch has
  max-resource1     ;; the max amount of resource1 for that patch
  resource2         ;; the amount of resource2 a patch has
  max-resource2     ;; the max amount of resource2 for that patch
  assigned-resource ;; the type of resource (resource1 or resource2) a patch has
]
...
to setup
  clear-all
  set energy-threshold 100                 ;; można przerobić na zmienną
  set replicate-chance random-float 100     ;; w Echo było jako suwak
  set mating-selectivity 0                 ;; w Echo było jako suwak
  set mutation-rate 10                     ;; w Echo było jako suwak
  setup-all-tags-list
  setup-creatures
  setup-patches
  reset-ticks
...
;;    For patches with pxcor < 0 (left half of the world), we use random-float 1 < 0.7 to make resource1 more likely (70% probability) and resource2 less likely (30% probability).
;;    For patches with pxcor >= 0 (right half of the world), we use random-float 1 < 0.7 to make resource2 more likely (70% probability) and resource1 less likely (30% probability).
to setup-patches
  ask patches [
    ifelse pxcor < 0 [
      ; More likely to have resource1 in the left half (west)
      ifelse random-float 1 < environmental-difference-threshold [
        set max-resource1 random 50
        set resource1 max-resource1
        set assigned-resource "resource1"
      ] [
        set max-resource2 random 50
        set resource2 max-resource2
        set assigned-resource "resource2"
      ]
    ] [
      ; More likely to have resource2 in the right half (east)
      ifelse random-float 1 < environmental-difference-threshold [
        set max-resource2 random 50
        set resource2 max-resource2
        set assigned-resource "resource2"
...
;; mozna ustawic probability threshold jako zmienna regulowana przez obserwatora

to recolor-patch  ;; patch procedure
  ifelse resource1 > 0 [
    set pcolor scale-color green resource1 0 100
  ] [
    set pcolor scale-color blue resource2 0 100
  ]
end

; inna wersja
;to recolor-patch
;  if (assigned-resource = "resource1") [
;    set pcolor scale-color green resource1 0 100
;  ]
;  if (assigned-resource = "resource2") [
;    set pcolor scale-color blue resource2 0 100
;  ]
;end
...
to setup-creatures
  ...
  [
    ...
    set mating initial-tag ;; one-of all-tags
    set size 1   ;; since the patch size is so big make the turtles smaller
...
to-report tag-color [tag]
  ;; report 5 + 2 * position tag all-tags ;; oryginalna wersja z Echo
  ; report 100 + 50 * ( count-letter "a" mating + 0.1) / ( count-letter "b" mating + 0.1) ;; parametry dobrane tak, żeby otrzymać jak najfajniejsze kolory
...
to replenish  ;; patch procedure
  if assigned-resource = "resource1" [
    if resource1 < max-resource1 [
      set resource1 resource1 + ((max-resource1 - resource1) * (replenish-speed / 100))
    ]
  ]
  if assigned-resource = "resource2" [
    if resource2 < max-resource2 [
      set resource2 resource2 + ((max-resource2 - resource2) * (replenish-speed / 100))
    ]
  ]
end
...
to grab-resources    ;; turtle procedure
  let unit-share count turtles-here * 5      ;; 5 is a number of loci in mating tag
  let count-a count-letter "a" mating
  let count-b count-letter "b" mating
;;  print word "mating tag: " mating     ;; tylko do sprawdzenia, czy literki dobrze są liczone
;;  print word "count a: " count-a
;;  print word "count b: " count-b

  if (energy < energy-threshold) [
    if (assigned-resource = "resource1") and (resource1 > 0) [
      if (count-a >= 2) [ ;; do przemyślenia
      set energy energy + ( resource1 / unit-share * count-a)
        set resource1 resource1 - ( resource1 / unit-share * count-a )]]
    if (assigned-resource = "resource2") and (resource2 > 0) [
      if (count-b >= 2) [  ;; do przemyślenia
      set energy energy + ( resource2 / unit-share * count-b)
      set resource2 resource2 - ( resource2 / unit-share * count-b)
    ]]
  ]
end
...
to replicate   ;; creature procedure
  if energy > energy-threshold and random-float 100 < replicate-chance ;; w Echo replicate-chance ustawiane suwakiem przez użytkownika
  [
...
;          ( match-score [mating] of agent2 [offense] of agent1 > mating-selectivity ) )
       ( ((count-letter "a" [mating] of agent1 - count-letter "a" [mating] of agent2) < 2) and
         ((count-letter "a" [mating] of agent1 - count-letter "a" [mating] of agent2) > -2) )
    [
      hatch 1
      [
...
;; powyzsza czesc jest niepotrzebna przy tagach rownej dlugosci
  report (map [ [t1 t2] -> one-of (list t1 t2)] tag1 tag2)
; można jeszcze pomyśleć nad dodaniem preferencji a/b adekwatnie do resource1/resource2
end
...
;  print word "offense: " [offense] of agent1
;  print word "defense: " [defense] of agent2
;  print word "a1a2: " a1a2
;  print word "offense: " [offense] of agent2
;  print word "defense: " [defense] of agent1
;  print word "a2a1: " a2a1

  ;; scale resulting scores so that all values are positive
  set a1a2 a1a2 + 10    ;; Echo: +6
  set a2a1 a2a1 + 10    ;; Echo: +6
...
;  print word "a2a1: " a2a1
  ...
  ;; of its match score
  set a1a2 (a1a2 / 20) * ([energy] of agent2)    ;; Echo: /12
  set a2a1 (a2a1 / 20) * ([energy] of agent1)    ;; Echo: /12
...
to-report match-score [tag1 tag2]
  ...
  foreach (range 0 5) [
    i ->
    let letter1 (item i tag1)
    let letter2 (item i tag2)
;    print word "i: " i
;    print word "tag1: " tag1
;    print word "letter1: " letter1
;    print word "tag2: " tag2
;    print word "letter2: " letter2

    ifelse letter1 = letter2 [
      set score (score - 2)
    ] [
      set score (score + 2)
    ]
...
479
132
environmental-difference-threshold
environmental-difference-threshold
0.5
1
0.7
