...
to grab-resources    ;; turtle procedure
  if (energy < energy-threshold) [
    if (assigned-resource = "resource1") and (resource1 > 0) [
      set energy energy + ( resource1 / count turtles-here)
      set resource1 resource1 - ( resource1 / count turtles-here )]
    if (assigned-resource = "resource2") and (resource2 > 0) [
      set energy energy + ( resource2 / count turtles-here)
      set resource2 resource2 - ( resource2 / count turtles-here )
      ]
  ]
end
